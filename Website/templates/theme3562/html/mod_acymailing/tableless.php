...
$document->addScriptdeclaration('(function($){$(document).ready(function(){$("#'.$formName.'").validate({wrapper:"mark",submitHandler:function(a){
...
<?php
	...
	<div class="acymailing_fulldiv" id="acymailing_fulldiv_<?php echo $formName; ?>" <?php echo $styleString; ?> >
		<form id="<?php echo $formName; ?>" action="<?php echo acymailing_route('index.php'); ?>" method="post" name="<?php echo $formName ?>" <?php if(!empty($fieldsClass->formoption)) echo $fieldsClass->formoption; ?> >
			...
			<div class="acymailing_form">
					...
					foreach($fieldsToDisplay as $oneField){
						...
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							...
							<span class="acyfield_<?php echo $oneField. (!$displayOutside? ' acy_requiredField':''); ?>"><input id="user_email_<?php echo $formName; ?>" class="inputbox" type="text" name="user[name]" <?php if(!empty($identifiedUser->userid)) echo ' disabled="disabled" value="'.$identifiedUser->name.'"'; ?><?php if(!$displayOutside) echo ' placeholder="'.$nameCaption.'"'; ?> required></span>
							<?php
				...
				<?php }
				 ...
				 if($params->get('showterms',false)){
					...
					<p class="acysubbuttons">
						...
						<input class="button subbutton btn btn-primary" type="submit" value="<?php $subtext = $params->get('subscribetextreg'); if(empty($identifiedUser->userid) OR empty($subtext)){ $subtext = $params->get('subscribetext',acymailing_translation('SUBSCRIBECAPTION')); } echo $subtext;  ?>" name="Submit"/>
						...
						<input class="button unsubbutton btn btn-inverse" type="button" value="<?php echo $params->get('unsubscribetext',acymailing_translation('UNSUBSCRIBECAPTION')); ?>" name="Submit" />
						<?php } ?>
					</p>
				</div>
			<?php
